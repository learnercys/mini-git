...
func Deserialize(data []byte) (*Blob, error) {
	...
	if nullIndex == -1 {
		return nil, fmt.Errorf("invalid blob data")
	}

	header := string(data[:nullIndex])
	if !bytes.HasPrefix(data, []byte("blob ")) {
		return nil, fmt.Errorf("invalid blob header: incorrect header")
	}

	sizeStr := header[5:]
	size, err := strconv.ParseInt(sizeStr, 10, 64)
	if err != nil {
		return nil, fmt.Errorf("invalid blob header: invalid size")
	}

	content := data[nullIndex+1:]
	if int64(len(content)) != size {
		return nil, fmt.Errorf("invalid blob data: incorrect content size")
	}
